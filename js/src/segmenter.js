...
//
// They diverge from the reference segmenter in ways that are recorded in its
// Canonical Algorithm Spec header — most of all the
 ...
 *  model-read of the reference segmenter's output over 4,251 bands found that of
 *  the 642 landing in [0.4, 0.6) of the page
 ...
 *  that read's 990 sub-0.6x bands were page numbers and watermarks, read correctly,
...
class LineSegmenter {
    ...
    async segment(imagePath) {
        ...
        for (let y = 0; y < height; y++) {
            ...
            // the reference segmenter dilates the mask vertically before taking the
            // profile and this one does not, so its break point is 5px to 8px, not 3px.
    ...
    // and its _extract_line sums `dilated`.
